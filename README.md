...
The honeypot engine **is fully functional** and captures real, live attacker traffic when deployed with open ports on a dedicated VM — see the [screen recording](https://claude.ai/chat/bc61b221-f600-4e5f-9b83-0309788e506b#) and [architecture walkthrough](https://claude.ai/chat/bc61b221-f600-4e5f-9b83-0309788e506b#architecture) below for what that looks like in practice.
...
| Step | What Happens |
| ----- | ----- |
| Attacker connects | Honeypot accepts the connection on a fake service (SSH, HTTP, FTP, MySQL) |
| Attacker tries credentials | System records username, password, and timing |
| Tarpit activated | Responses are deliberately slowed to waste the attacker's time and collect more behavioral data |
| Data logged | IP, payload, service, and timestamp saved to PostgreSQL |
| ML classification | Attack type identified — brute force, SQL injection, port scan, credential stuffing, slow probe |
| Repeat attacker | IP automatically added to the blocklist |
| Dashboard updated | Live battlefield visual shows the attack in real time |
...
| Layer | Concept | Problem Solved | Where Used |
| ----- | ----- | ----- | ----- |
| 1 | `asyncio` | I/O-bound — thousands of concurrent connections | Honeypot network handling |
| 2 | Threading | Concurrent tasks, shared memory | Parallel isolation per service (SSH/HTTP/FTP/DB) |
| 3 | Multiprocessing | CPU-bound — true parallelism | ML classification, run in batches |
| Bonus | NumPy vectorization | Data parallelism | Threat score computation |
...
| Feature | Signal |
| ----- | ----- |
| Requests per minute | High → automated bot |
| Unique passwords tried | High → brute force |
| Services hit | Many → port scanner |
| Payload size | Large → exploit attempt |
| Time between requests | Regular → scripted |
...
| Deployment (public demo) | Render (PostgreSQL, Flask API, React static site) |
| Networking (local dev only) | ngrok |
...
| ----- | ----- | ----- |
| PostgreSQL | Managed database (free tier) | Free tier databases expire 30 days after creation — known limitation, not a bug, planned around rather than avoided |
| Flask API | Web service (Docker, `backend/Dockerfile`) | Reads `FLASK_PORT` from env, binds `0.0.0.0` |
| React frontend | Static site | Built with `REACT_APP_API_URL` pointed at the live API service |
