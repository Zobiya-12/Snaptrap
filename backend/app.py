...
import jwt, datetime, os, json, csv, io, psycopg2, psycopg2.extras, time, pickle
...
load_dotenv()
app = Flask(__name__)
app.secret_key = os.getenv('FLASK_SECRET', 'snaptrap_secret_2024')
CORS(app, resources={r"/api/*": {"origins": "*"}}, supports_credentials=False)
JWT_SECRET = os.getenv('FLASK_SECRET', 'snaptrap_secret_2024')
JWT_ALGO = 'HS256'
...
# Default service configs per org (fallback if not in DB)
DEFAULT_SERVICES = [
...
def make_token(org_id, email, role, name=''):
    return jwt.encode({'org_id':org_id,'email':email,'role':role,'name':name,
        'exp':datetime.datetime.utcnow()+datetime.timedelta(hours=24)}, JWT_SECRET, algorithm=JWT_ALGO)
...
def token_required(f):
    ...
    def dec(*a,**kw):
        ...
        try:
            d = decode_token(tk)
            request.org_id=d['org_id']; request.role=d['role']
            request.email=d['email']; request.uname=d.get('name','')
...
def sa_required(f):
    ...
    def dec(*a,**kw):
        ...
        try:
            d = decode_token(tk)
            if d['role']!='superadmin': return jsonify({'error':'Superadmin only'}),403
            request.org_id=d['org_id']; request.role=d['role']; request.email=d.get('email','')
        except: return jsonify({'error':'Invalid token'}),401
        return f(*a,**kw)
    return dec
...
def signup():
    d=request.json or {}
    name=d.get('name','').strip(); email=d.get('email','').strip(); pwd=d.get('password','').strip()
    if not all([name,email,pwd]): return jsonify({'error':'All fields required'}),400
    import re
    if len(pwd)<8: return jsonify({'error':'Password min 8 chars'}),400
    if not re.search(r'[A-Z]',pwd): return jsonify({'error':'Need uppercase letter'}),400
    if not re.search(r'[0-9]',pwd): return jsonify({'error':'Need a number'}),400
    import secrets; at=secrets.token_hex(16)
    try:
        conn=get_conn(); cur=conn.cursor()
        cur.execute("INSERT INTO organisations(name,email,password,role,agent_token) VALUES(%s,%s,%s,'org',%s) RETURNING id",
                    (name,email,pwd,at))
        oid=cur.fetchone()[0]; conn.commit(); cur.close(); conn.close()
        # seed default service config
        _seed_services(oid)
        return jsonify({'token':make_token(oid,email,'org',name),'org_id':oid,'name':name,'email':email,'role':'org','agent_token':at}),201
    except Exception as e:
        if 'unique' in str(e).lower(): return jsonify({'error':'Email already registered'}),409
        return jsonify({'error':str(e)}),500

@app.route('/api/auth/login', methods=['POST'])
def login():
    d=request.json or {}
    email=d.get('email','').strip(); pwd=d.get('password','').strip()
    if not all([email,pwd]): return jsonify({'error':'Email and password required'}),400
    try:
        conn=get_conn(); cur=conn.cursor()
        # Check organisations table first
        cur.execute("SELECT id,name,email,role,agent_token FROM organisations WHERE email=%s AND password=%s",(email,pwd))
        row=cur.fetchone()
        if row:
            oid,nm,em,role,at=row; cur.close(); conn.close()
            return jsonify({'token':make_token(oid,em,role,nm),'org_id':oid,'name':nm,'email':em,'role':role,'agent_token':at})
        # Check red_team_accounts table
        cur.execute("SELECT id,name,email,org_id FROM red_team_accounts WHERE email=%s AND password=%s",(email,pwd))
        rt=cur.fetchone(); cur.close(); conn.close()
        if rt:
            rid,nm,em,org_id=rt
            return jsonify({'token': make_token(org_id, em, 'redteam', nm), 'org_id': org_id,'name':nm,'email':em,'role':'redteam','agent_token':None})
...
@token_required
def me():
    return jsonify({'org_id':request.org_id,'email':request.email,'role':request.role,'name':request.uname})
...
@token_required
def attacks_since():
    """Core live-polling endpoint: returns attacks with id > since_id, scoped to org."""
...
def get_attack(aid):
    try:
        conn=get_conn(); cur=conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("SELECT a.*,p.predicted_type,p.confidence FROM attacks a LEFT JOIN ml_predictions p ON p.attack_id=a.id WHERE a.id=%s AND (%s='superadmin' OR a.org_id=%s)",
...
def delete_attack(aid):
    try:
        conn=get_conn(); cur=conn.cursor()
        cur.execute("DELETE FROM attacks WHERE id=%s AND (%s='superadmin' OR org_id=%s)",(aid,request.role,request.org_id))
...
def get_blocklist():
    try:
        conn=get_conn(); cur=conn.cursor()
        cur.execute("SELECT ip_address,risk_level,total_hits,last_seen FROM attackers WHERE risk_level='high' AND org_id=%s ORDER BY total_hits DESC",(request.org_id,))
        rows=cur.fetchall(); cur.close(); conn.close()
...
def _seed_services(org_id):
    """Create default service rows for a new org."""
    try:
        conn=get_conn(); cur=conn.cursor()
...
def update_svc_config():
    """Update port for a service. Body: {service_name, port}"""
    d=request.json or {}
...
@token_required
def regen_token():
    import secrets; nt=secrets.token_hex(16)
    try:
        conn=get_conn(); cur=conn.cursor()
...
def dl_agent():
    try:
        ...
        # Try real agent.py on disk first
        ...
        if os.path.exists(ap):
            ...
            # Clean string concatenation — no f-string brace escaping issues
            script  = "#!/usr/bin/env python3\n"
            script += "# SNAPTRAP Agent — pre-configured for " + org_name + "\n"
            script += "# Edit SERVER_URL to point at your SNAPTRAP VM, then run.\n\n"
            ...
            script += "    try:\n"
            script += "        r = requests.post(\n"
            script += "            SERVER_URL,\n"
            script += '            json={"attacker_ip": attacker_ip, "service": service,\n'
            script += '                  "port": port, "payload": payload,\n'
            script += '                  "threat_score": threat_score, "attack_type": attack_type},\n'
            script += '            headers={"X-Agent-Token": AGENT_TOKEN},\n'
            script += "            timeout=10\n"
            script += "        )\n"
            script += '        print("[SENT]", service, attacker_ip, "score=" + str(threat_score), "->", r.status_code)\n'
            script += "    except Exception as e:\n"
            script += '        print("[ERR]", str(e))\n\n'
            script += 'if __name__ == "__main__":\n'
            script += '    print("SNAPTRAP Agent ready for ' + org_name + '")\n'
            script += '    print("Token:", AGENT_TOKEN[:8] + "...")\n'
            script += '    print("Edit SERVER_URL and wire send_attack() to your honeypot handlers.")\n'
...
def extract_features(payload,service='unknown',threat_score=0):
    ...
    return [[L,digits,specials,entropy,threat_score,int('ssh' in service.lower()),int('http' in service.lower()),int('ftp' in service.lower()),sql_sc,int(sql_sc>0)]]
...
def predictions():
    try:
        conn=get_conn(); cur=conn.cursor()
        cur.execute("SELECT p.predicted_type,COUNT(*) c,AVG(p.confidence) ac FROM ml_predictions p JOIN attacks a ON a.id=p.attack_id WHERE a.org_id=%s GROUP BY p.predicted_type ORDER BY c DESC",(request.org_id,))
        rows=cur.fetchall()
        cur.execute("SELECT COUNT(*) FROM ml_predictions p JOIN attacks a ON a.id=p.attack_id WHERE a.org_id=%s",(request.org_id,))
...
def classify_payload():
    ...
    try:
        ...
        pred=model.predict(features)[0]; proba=model.predict_proba(features)[0]
        classes=model.classes_.tolist()
        return jsonify({'predicted_type':pred,'confidence':round(float(max(proba)),3),'breakdown':{c:round(float(p),3) for c,p in zip(classes,proba)}})
    except Exception as e: return jsonify({'error':str(e)}),500
...
def agent_report():
    ...
    try:
        ...
        aid=insert_attack(d.get('attacker_ip','unknown'),d.get('service','unknown'),d.get('port',0),
                          d.get('payload',''),d.get('threat_score',0),d.get('attack_type','unknown'),org_id)
...
def admin_create_org():
    d=request.json or {}
    name=d.get('name','').strip(); email=d.get('email','').strip(); pwd=d.get('password','').strip(); role=d.get('role','org')
    if not all([name,email,pwd]): return jsonify({'error':'name,email,password required'}),400
    import secrets; at=secrets.token_hex(16)
    try:
        conn=get_conn(); cur=conn.cursor()
        cur.execute("INSERT INTO organisations(name,email,password,role,agent_token) VALUES(%s,%s,%s,%s,%s) RETURNING id",(name,email,pwd,role,at))
        oid=cur.fetchone()[0]; conn.commit(); cur.close(); conn.close()
        _seed_services(oid)
...
def admin_del_org(oid):
    ...
    try:
        conn=get_conn(); cur=conn.cursor()
        cur.execute("DELETE FROM attacks WHERE org_id=%s",(oid,))
        cur.execute("DELETE FROM attackers WHERE org_id=%s",(oid,))
        cur.execute("DELETE FROM service_status WHERE org_id=%s",(oid,))
...
def admin_patch_org(oid):
    ...
    if 'role' in d: updates.append("role=%s"); params.append(d['role'])
    if 'password' in d: updates.append("password=%s"); params.append(d['password'])
...
def admin_db():
    try:
        ...
        for t in ['attacks','attackers','organisations','ml_predictions','benchmark_results']:
...
def cross_org_ips():
    try:
        conn=get_conn(); cur=conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("""SELECT attacker_ip,COUNT(DISTINCT org_id) org_count,COUNT(*) total_hits,MAX(threat_score) max_score,
                   array_agg(DISTINCT service ORDER BY service) services
                   FROM attacks GROUP BY attacker_ip HAVING COUNT(DISTINCT org_id)>1
...
def get_ngrok_url_admin():
    ...
    try:
        ...
        url = f'http://{ngrok_host}:4044/api/tunnels'
...
def get_ngrok_url():
    ...
    try:
        ...
        if not tunnels:
            return jsonify({'error': 'No active ngrok tunnels'}), 404
        return jsonify({'url': tunnels[0]['public_url']})
    except Exception as e:
        return jsonify({'error': str(e)}), 503
    
# ── CSV EXPORT — auth via Bearer header, returns downloadable CSV ──────
...
def export_csv():
    ...
    try:
        ...
        return Response(buf.getvalue(),mimetype='text/csv',
            headers={'Content-Disposition':f'attachment; filename="{fname}"','Access-Control-Expose-Headers':'Content-Disposition'})
    except Exception as e: return jsonify({'error':str(e)}),500
...
def prometheus_metrics():
    """Prometheus text format — scraped by prometheus container every 15s."""
    try:
        stats = get_stats()
        conn = get_conn(); cur = conn.cursor()
...
@token_required
def list_redteam():
    if request.role not in ('org', 'superadmin'):
        return jsonify({'error': 'Forbidden'}), 403
    try:
        conn = get_conn(); cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("""
            SELECT r.id, r.name, r.email, r.created_at,
                   COUNT(rn.id) AS run_count
...
def create_redteam():
    ...
    try:
        conn = get_conn(); cur = conn.cursor()
        cur.execute(
            "INSERT INTO red_team_accounts (name,email,password,org_id) VALUES (%s,%s,%s,%s) RETURNING id",
            (name, email, password, request.org_id)
        )
        rid = cur.fetchone()[0]
        conn.commit(); cur.close(); conn.close()
...
# ── SIMULATOR — persists attacks to DB so stats/threat level update ───
...
def simulate_attacks():
    ...
    for a in attacks:
        aid = insert_attack(
            a.get('attacker_ip', '10.0.0.1'),
            a.get('service', 'SSH'),
            a.get('port', 22),
            a.get('payload', '[simulated]'),
            a.get('threat_score', 50),
            a.get('attack_type', 'unknown'),
            request.org_id
        )
        ids.append(aid)
...
def redteam_simulate():
    if request.role != 'redteam':
        return jsonify({'error': 'Red team only'}), 403
    d = request.json or {}
    mode = d.get('mode', 'demo')
    total = 700 if mode == 'benchmark' else 50
    detected = int(total * 0.87)
...
def health():
    return jsonify({'status':'ok','service':'SNAPTRAP API','version':'2.1.0'})
